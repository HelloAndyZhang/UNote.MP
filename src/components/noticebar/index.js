...
export default class Noticebar extends Component {
    ...
    render() {
        ...
        const style = {}
        const innerClassName = ['noticebar__content-inner']
        if (marquee) {
            close = false
        ...
        const classObject = {
            'noticebar--marquee': marquee,
            'noticebar--weapp': marquee && (this.state.isWEAPP || this.state.isALIPAY),
            'noticebar--single': !marquee && single,
        }

        const iconClass = ['icon']
        if (icon) iconClass.push(`icon-${icon}`)
        ...
        return (
            ...
            >
                {close && (
                    <View className='noticebar__close' onClick={this.onClose.bind(this)}>
                        <Text className='icon icon-close'></Text>
                    </View>
                )}
                <View className='noticebar__content'>
                    {icon && (
                        <View className='noticebar__content-icon'>
                            {/* start hack 百度小程序 */}
                            <Text className={classNames(iconClass, iconClass)}></Text>
                        </View>
                    )}
                    <View className='noticebar__content-text'>
                        <View animation={this.state.animationData} className={classNames(innerClassName)} style={style}>{this.props.children}</View>
                    </View>
                </View>
                {showMore && (
                    <View className='noticebar__more' onClick={this.onGotoMore.bind(this)}>
                        <Text className='text'>{_moreText}</Text>
                        <View className='noticebar__more-icon'>
                            <Text className='icon icon-chevron-right'></Text>
                        </View>
                    </View>
                )}
            </View>
